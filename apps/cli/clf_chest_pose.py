...
from halibut.MobileNetGray import MobileNet
...
parser = argparse.ArgumentParser(description="Run CXR view classifier") 
...
prediction = model.predict(np.expand_dims(np.expand_dims(image, axis=2), axis=0))[0][0]

if prediction >= 0.5: 
    print(">>PREDICTED VIEW: FRONTAL ({})<<".format( round(prediction, 2) ))
else:
    print(">>PREDICTED VIEW: LATERAL ({})<<".format( round(1.0-prediction, 2) ))
